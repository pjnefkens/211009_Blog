...
class FMMFlowEntry : Hashable, CustomStringConvertible {
    ...
    /**
     Implement custom hash function and only add the properties that combined make a single entry unique.
     The produced Hash value is different for each time you run the code. Never store the hash for later retrieval.
     */
...
class FMMIPFlow  : FMMFlowEntry  {
    ...
    // convenience constructor to initalize a indexed entry based on a Base flow entry
    ...
    // override the hash function to only index on srcIP and dstIP combination
